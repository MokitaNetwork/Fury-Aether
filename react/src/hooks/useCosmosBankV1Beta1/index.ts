...
export default function useCosmosBankV1Beta1() {
  ...
  const QuerySupplyOf = (denom: string,  options: any) => {
    const key = { type: 'QuerySupplyOf',  denom };    
    return useQuery([key], () => {
      const { denom } = key
      return  client.CosmosBankV1Beta1.query.querySupplyOf(denom).then( res => res.data );
    }, options);
  }
  ...
  return {QueryBalance,QueryAllBalances,QuerySpendableBalances,QueryTotalSupply,QuerySupplyOf,QueryParams,QueryDenomMetadata,QueryDenomsMetadata,
  }
}