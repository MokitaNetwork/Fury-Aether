...
import (
	...
	"github.com/mokitanetwork/aether/app/wasm"
...
func TestWhitelistAssetLocker(t *testing.T) {
	...
	aether, ctx := SetupCustomApp()
	AddAppAsset(aether, *ctx)
	querier := wasm.NewQueryPlugin(&aether.AssetKeeper,
		&aether.LockerKeeper,
		&aether.TokenmintKeeper,
		&aether.Rewardskeeper,
		&aether.CollectorKeeper,
		&aether.LiquidationKeeper,
		&aether.EsmKeeper,
		&aether.VaultKeeper,
		&aether.LendKeeper,
		&aether.LiquidityKeeper)
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := wasm.WhiteListAsset(aether.LockerKeeper, *ctx, actor.String(), tc.msg)
...
func TestAddMsgAddExtendedPairsVault(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgAddExtendedPairsVault(aether.AssetKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgSetCollectorLookupTable(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgSetCollectorLookupTable(aether.CollectorKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgSetAuctionMappingForApp(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgSetAuctionMappingForApp(aether.CollectorKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgUpdateCollectorLookupTable(t *testing.T) {
	...
	aether, ctx := SetupCustomApp()
	AddPair(aether, *ctx)
	AddCollectorLookuptable(aether, *ctx)
	querier := wasm.NewQueryPlugin(&aether.AssetKeeper,
		&aether.LockerKeeper,
		&aether.TokenmintKeeper,
		&aether.Rewardskeeper,
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgUpdateCollectorLookupTable(aether.CollectorKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgUpdatePairsVault(t *testing.T) {
	...
	aether, ctx := SetupCustomApp()
	AddPair(aether, *ctx)
	AddExtendedPairVault(aether, *ctx)
	querier := wasm.NewQueryPlugin(&aether.AssetKeeper,
		&aether.LockerKeeper,
		&aether.TokenmintKeeper,
		&aether.Rewardskeeper,
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgUpdatePairsVault(aether.AssetKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgWhitelistAppIDLiquidation(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgWhitelistAppIDLiquidation(aether.LiquidationKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgRemoveWhitelistAppIDLiquidation(t *testing.T) {
	...
	aether, ctx := SetupCustomApp()
	AddPair(aether, *ctx)
	WhitelistAppIDLiquidation(aether, *ctx)
	querier := wasm.NewQueryPlugin(&aether.AssetKeeper,
		&aether.LockerKeeper,
		&aether.TokenmintKeeper,
		&aether.Rewardskeeper,
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgRemoveWhitelistAppIDLiquidation(aether.LiquidationKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgAddAuctionParams(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, found)
			err := wasm.MsgAddAuctionParams(aether.AuctionKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestMsgBurnGovTokensForApp(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			err := wasm.MsgBurnGovTokensForApp(aether.TokenmintKeeper, *ctx, actor, tc.msg)
			if tc.isErrorExpected {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
