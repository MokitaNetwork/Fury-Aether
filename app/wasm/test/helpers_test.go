...
import (
	...
	"github.com/mokitanetwork/aether/x/tokenmint/keeper"
)

func SetupCustomApp() (*app.App, *sdk.Context) {
	aether, ctx := CreateTestInput()
	return aether, ctx
}

func CreateTestInput() (*app.App, *sdk.Context) {
	aether := app.Setup(false)
	ctx := aether.BaseApp.NewContext(false, tmproto.Header{Height: 1, ChainID: "aether-1", Time: time.Now().UTC()})
	return aether, &ctx
}

func FundAccount(t *testing.T, ctx sdk.Context, aether *app.App, acct sdk.AccAddress) {
	err := simapp.FundAccount(aether.BankKeeper, ctx, acct, sdk.NewCoins(
		sdk.NewCoin("uaeth", sdk.NewInt(10000000000)),
	))
...
func AddExtendedPairVault(app *app.App, ctx1 sdk.Context) {
	...
	for _, tc := range []struct {
		...
		{
			"Add Extended Pair Vault : aeth fusd",
