...
import (
	...
	"github.com/mokitanetwork/aether/x/asset/keeper"
...
func (s *KeeperTestSuite) TestAddAssetRecords() {
	...
	for _, tc := range []struct {
		...
		{
			"Add Asset : Duplicate Denom 2 osmo uaeth",
			assetTypes.Asset{
				Name:                  "OSMO",
				Denom:                 "uaeth",
				Decimals:              sdk.NewInt(1000000),
...
func (s *KeeperTestSuite) TestWasmUpdatePairsVault() {
	...
	for _, tc := range []struct {
		...
		{
			"Update Extended Pair Vault : aeth fusd",
...
func (s *KeeperTestSuite) TestAddExtendedPairVault() {
	...
	for _, tc := range []struct {
		...
		{
			"Add Extended Pair Vault : Duplicate PairName fusd aeth",
