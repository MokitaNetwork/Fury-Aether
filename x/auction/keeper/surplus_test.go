...
import (
	...
	tokenmintKeeper1 "github.com/mokitanetwork/aether/x/tokenmint/keeper"
...
func (s *KeeperTestSuite) TestSurplusActivator() {
	...
	err2 := k.FundModule(*ctx, auctionTypes.ModuleName, "ufusd", 1000000000)
	s.Require().NoError(err2)
	err3 := s.app.BankKeeper.SendCoinsFromModuleToModule(*ctx, auctionTypes.ModuleName, collectorTypes.ModuleName, sdk.NewCoins(sdk.NewCoin("ufusd", sdk.NewIntFromUint64(1000000000))))
...
func (s *KeeperTestSuite) TestSurplusBid() {
	...
	for _, tc := range []struct {
		...
		s.Run(tc.name, func() {
			...
			s.Require().NoError(err)
			beforeCmstBalance, err := s.getBalance(tc.msg.Bidder, "ufusd")
			s.Require().NoError(err)
			previousUserAddress := ""
			...
			if tc.isErrorExpected {
				...
				s.Require().NoError(err)
				afterCmstBalance, err := s.getBalance(tc.msg.Bidder, "ufusd")
				s.Require().NoError(err)
...
func (s *KeeperTestSuite) TestCloseSurplusAuction() {
	...
	for _, tc := range []struct {
		...
		s.Run(tc.name, func() {
			beforeCmstBalance, err := s.getBalance(winnerAddress, "ufusd")
			s.Require().NoError(err)
			...
			afterCmstBalance, err := s.getBalance(winnerAddress, "ufusd")
			// s.Require().NoError(err)
