...
func (s *KeeperTestSuite) TestDebtBid() {
	...
	for _, tc := range []struct {
		...
		{
			"TestDebtBid : Less ExpectedUserToken AppID 1 Asset 2 21000ufusd",
			auctionTypes.MsgPlaceDebtBidRequest{
				AuctionId:         1,
				Bidder:            userAddress1,
				Bid:               ParseCoin("2000000uharbor"),
				ExpectedUserToken: ParseCoin("21000ufusd"),
				AppId:             appID,
		...
		{
			"TestDebtBid : More ExpectedUserToken AppID 1 Asset 2 200001ufusd",
			auctionTypes.MsgPlaceDebtBidRequest{
				AuctionId:         1,
				Bidder:            userAddress1,
				Bid:               ParseCoin("2000000uharbor"),
				ExpectedUserToken: ParseCoin("200001ufusd"),
				AppId:             appID,
				AuctionMappingId:  auctionMappingID,
			},
			1,
		...
		},
		{
			...
			auctionTypes.MsgPlaceDebtBidRequest{
				...
				ExpectedUserToken: ParseCoin("200000ufusd"),
				AppId:             appID,
				AuctionMappingId:  auctionMappingID,
			},
			1,
		...
		s.Run(tc.name, func() {
			...
			if tc.bidID != uint64(1) {
				...
				beforeCmstBalance2, err = s.getBalance(previousUserAddress, "ufusd")
				s.Require().NoError(err)
			...
			if tc.isErrorExpected {
				...
				if tc.bidID != uint64(1) {
					afterCmstBalance2, err := s.getBalance(previousUserAddress, "ufusd")
					s.Require().NoError(err)
