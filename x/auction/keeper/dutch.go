...
func (k Keeper) DutchActivator(ctx sdk.Context, lockedVault liquidationtypes.LockedVault) error {
	if lockedVault.Kind == nil {
		...
		assetIn, _ := k.asset.GetAsset(ctx, pair.AssetIn) // collateral(aeth)

		assetOut, _ := k.asset.GetAsset(ctx, pair.AssetOut) // debt(fusd)

		outflowToken := sdk.NewCoin(assetIn.Denom, lockedVault.AmountIn) // aeth
		inflowToken := sdk.NewCoin(assetOut.Denom, sdk.ZeroInt())        // fusd
...
func (k Keeper) StartDutchAuction(
	ctx sdk.Context,
	outFlowToken sdk.Coin, // aeth
	inFlowToken sdk.Coin, // fusd
	appID uint64,
	assetInID uint64, // fusd
	assetOutID uint64, // aeth
	...
	auction := auctiontypes.DutchAuction{
		...
		AppId:                     appID,
		AssetInId:                 assetInID,  // fusd
		AssetOutId:                assetOutID, // aeth
...
func (k Keeper) PlaceDutchAuctionBid(ctx sdk.Context, appID, auctionMappingID, auctionID uint64, bidder sdk.AccAddress, bid sdk.Coin, max sdk.Dec) error {
	...
	outFlowTokenCurrentPrice := auction.OutflowTokenCurrentPrice // aeth
	inFlowTokenCurrentPrice := auction.InflowTokenCurrentPrice   // fusd

	slice := bid.Amount // aeth

	a := auction.InflowTokenTargetAmount.Amount
	b := auction.InflowTokenCurrentAmount.Amount
	tab := a.Sub(b) // leftover fusd

	// owe is $aeth to be given to user
	...
	// required target fusd to raise in usd * 10**-12
	// here we are multiplying each uaeth with uusd so aeth tokens price will be calculated amount * 10**-12
	...
	// convert amountLeft to uusd from pusd(10**-12) so we can compare dust and amountLeft in UUSD . this happens by converting uaeth to aeth

	// check if bid in usd*10**-12 is greater than required target fusd in usd*10**-12
	// if user wants to buy more than target fusd then user should be sold only required fusd amount
	// so we need to divide tab by outflow token current price and we get outflowtoken amount to be sold to user
	// if user is not buying more than required fusd then we check for dust
	...
	// collateral not over but target fusd reached then send remaining collateral to owner
	...
	if auction.InflowTokenCurrentAmount.IsGTE(auction.InflowTokenTargetAmount) {
		// send left overcollateral to vault owner as target fusd reached and also
...
func (k Keeper) CloseDutchAuction(
	...
	// burn and send target FUSD to collector
		...
		_ = utils.ApplyFuncIfNoError(ctx, func(ctx sdk.Context) error {
			...
			if ctx.BlockTime().After(dutchAuction.EndTime) {
				...
				if status {
					...
					// if not create new vault of user with aeth fusd
