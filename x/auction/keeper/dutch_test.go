...
import (
	...
	vaultKeeper1 "github.com/mokitanetwork/aether/x/vault/keeper"
...
func (s *KeeperTestSuite) AddPairAndExtendedPairVault1() {
	...
	for _, tc := range []struct {
		...
		{
			"Add Pair , Extended Pair Vault : aeth fusd",
			assetTypes.Pair{
				AssetIn:  1,
				AssetOut: 2,
			},
...
func (s *KeeperTestSuite) TestDutchBid() {
	...
	for _, tc := range []struct {
		...
		{
			"Place Dutch Bid : collateral max price less than current price 1000uaeth max 1.1",
			auctionTypes.MsgPlaceDutchBidRequest{
				AuctionId:        1,
				Bidder:           userAddress1,
				Amount:           ParseCoin("1000uaeth"),
		...
		{
			"Place Dutch Bid : collateral max price more than current price 1000uaeth max 1.4",
			auctionTypes.MsgPlaceDutchBidRequest{
				AuctionId:        1,
				Bidder:           userAddress1,
				Amount:           ParseCoin("1000uaeth"),
		...
		s.Run(tc.name, func() {
			...
			s.Require().NoError(err)
			beforeCmdxBalance, err := s.getBalance(userAddress1, "uaeth")
			s.Require().NoError(err)
			beforeCmstBalance, err := s.getBalance(userAddress1, "ufusd")
			s.Require().NoError(err)
			...
			if tc.isErrorExpected {
				...
				afterCmdxBalance, err := s.getBalance(userAddress1, "uaeth")
				s.Require().NoError(err)
				afterCmstBalance, err := s.getBalance(userAddress1, "ufusd")
				s.Require().NoError(err)
				...
				userOutflowCoin := sdk.NewCoin("ufusd", userReceivableAmount)
...
func (s *KeeperTestSuite) TestCloseDutchAuctionWithProtocolLoss() {
	...
	err1 := k.FundModule(*ctx, auctionTypes.ModuleName, "ufusd", 10000000)
	s.Require().NoError(err1)
	err = s.app.BankKeeper.SendCoinsFromModuleToModule(*ctx, auctionTypes.ModuleName, collectorTypes.ModuleName, sdk.NewCoins(sdk.NewCoin("ufusd", sdk.NewInt(10000000))))
	s.Require().NoError(err)
