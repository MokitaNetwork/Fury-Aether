...
import (
	...
	"github.com/mokitanetwork/aether/x/esm/expected"
	...
	"github.com/mokitanetwork/aether/x/esm/types"
...
func (k Keeper) CalculateCollateral(ctx sdk.Context, appID uint64, amount sdk.Coin, from string) error {
	...
	// Eg. If user brought $20 of FUSD (totalDebtAssetWorth) and if 2 collateral exist as AETH(80%) and ATOM(20%) then 20% of tokens that
	// will be paid will be in terms of ATOM tokens , rest AETH.
	...
	for _, tokenData := range allAssetToAmtData {
		...
		if tokenData.IsCollateral && !tokenData.Amount.IsZero() {
			...
			tokenShare := totalDebtAssetWorth.Mul(tokenData.Share) //$FUSD Multiplied with Share of collateral give $share of collateral
