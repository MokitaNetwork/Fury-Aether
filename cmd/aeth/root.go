...
func NewRootCmd() (*cobra.Command, aether.EncodingConfig) {
	var (
		config  = aether.MakeEncodingConfig()
		context = client.Context{}.
			...
			WithHomeDir(aether.DefaultNodeHome)
	)
...
func initRootCmd(rootCmd *cobra.Command, encoding aether.EncodingConfig) {
	rootCmd.AddCommand(
		genutilcli.InitCmd(aether.ModuleBasics, aether.DefaultNodeHome),
		genutilcli.CollectGenTxsCmd(banktypes.GenesisBalancesIterator{}, aether.DefaultNodeHome),
		genutilcli.GenTxCmd(aether.ModuleBasics, encoding.TxConfig, banktypes.GenesisBalancesIterator{}, aether.DefaultNodeHome),
		genutilcli.ValidateGenesisCmd(aether.ModuleBasics),
		AddGenesisAccountCmd(aether.DefaultNodeHome),
		AddGenesisWasmMsgCmd(aether.DefaultNodeHome),
		tmcli.NewCompletionCmd(rootCmd, true),
		testnetCmd(aether.ModuleBasics, banktypes.GenesisBalancesIterator{}),
		debug.Cmd(),
	)

	server.AddCommands(rootCmd, aether.DefaultNodeHome, appCreatorFunc, appExportFunc, addModuleInitFlags)
	rootCmd.AddCommand(
		rpc.StatusCommand(),
		queryCommand(),
		txCommand(),
		keys.Commands(aether.DefaultNodeHome),
	)
}
...
func queryCommand() *cobra.Command {
	...
	aether.ModuleBasics.AddQueryCommands(cmd)
...
func txCommand() *cobra.Command {
	...
	aether.ModuleBasics.AddTxCommands(cmd)
	cmd.PersistentFlags().String(flags.FlagChainID, "", "The network chain ID")
...
func appCreatorFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, options servertypes.AppOptions) servertypes.Application {
	...
	return aether.New(
		...
		aether.MakeEncodingConfig(),
		options,
		aether.GetWasmEnabledProposals(),
		wasmOpts,
...
func appExportFunc(logger log.Logger, db tmdb.DB, tracer io.Writer, height int64,
	...
	var app *aether.App
	if height != -1 {
		app = aether.New(logger, db, tracer, false, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, aether.GetWasmEnabledProposals(), emptyWasmOpts)

		if err := app.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		app = aether.New(logger, db, tracer, true, map[int64]bool{}, homePath, cast.ToUint(options.Get(server.FlagInvCheckPeriod)), config, options, aether.GetWasmEnabledProposals(), emptyWasmOpts)
	}
