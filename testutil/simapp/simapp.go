...
func New(dir string) *aether.App {
	var (
		db       = tmdb.NewMemDB()
		logger   = log.NewNopLogger()
		encoding = aether.MakeEncodingConfig()
	)

	a := aether.New(logger, db, nil, true, map[int64]bool{}, dir, 0, encoding,
		simapp.EmptyAppOptions{}, aether.GetWasmEnabledProposals(), aether.EmptyWasmOpts)
